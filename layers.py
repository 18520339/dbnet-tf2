...
class ConvBnRelu(tf.keras.layers.Layer):
    def __init__(self, filters=64, kernel_size=3, name='ConvBnRelu', **kwargs):
...
class DeConvMap(tf.keras.layers.Layer):
    def __init__(self, filters=64, name='DeConvMap', **kwargs):
        ...
        self.conv_bn = ConvBnRelu(filters, kernel_size=3)
        self.deconv1 = Conv2DTranspose(filters, kernel_size=2, strides=2)
        self.bn = BatchNormalization()
        self.relu = ReLU()
